...
class BufferTests: XCTestCase {
    func testBuffer() {
        var buffer = Buffer<UInt8>(size: 4)
        ...
        buffer = try! Buffer(data: Data(bytes: UnsafePointer<UInt8>(raw), count: 4))
    ...
    func testBufferFromHex() {
        var buffer = try! Buffer<UInt8>(hexData: "000000")
        XCTAssertEqual(buffer.values, [0,0,0])

        buffer = try! Buffer<UInt8>(hexData: "0A0B0C")
        XCTAssertEqual(buffer.values, [10,11,12])

        buffer = try! Buffer<UInt8>(hexData: "0a0b0c")
        XCTAssertEqual(buffer.values, [10,11,12])

        buffer = try! Buffer<UInt8>(hexData: "f34481ec3cc627bacd5dc3fb08f273e6")
    ...
    func testBuffer16Bit() {
        ...
        let raw : [UInt8] = [65,00,66,00,67,00,68,00] // ABCD
        buffer = try! Buffer(data: Data(bytes: UnsafePointer<UInt8>(raw), count: raw.count))
        XCTAssertEqual(buffer.values, [65,66,67,68])
        let data = buffer.data
