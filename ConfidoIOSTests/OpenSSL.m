...
+ (nullable NSData *)generateCSRWithPrivateKeyData:(nonnull NSData *)privateKeyData
    ...
    @try {
        ...
        evpPrivKey = [self createEVP_PKEYfromData:privateKeyData error:&localError];
...
+ (nullable OpenSSLIdentity *)pkcs12IdentityWithPrivateKeyData:(nonnull NSData *)privateKeyData
    ...
    @try {
        ...
        cert_privkey = [self createEVP_PKEYfromData:privateKeyData error:&localError];
        if (cert_privkey == NULL) {
            if (error != NULL) { *error = [self internalError:@"Could not extract private key (createEVP_PKEYfromData failed)"]; }
            return nil;
        }

        cert = [self createX509withPEMEncodedCertificateData:certificateData];
        if (cert == NULL) {
